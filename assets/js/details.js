    ...
    container.innerHTML = `
      <article class="card ${student.resultat}">
        ...
        <p><strong>Résultat:</strong> ${
          student.resultat.charAt(0).toUpperCase() + student.resultat.slice(1)
        }</p>
        ${
          student.resultat === "renvoye" && student.cause
            ? `<p><strong>Cause du renvoi:</strong> ${student.cause}</p>`
            : ""
        }
        ${
          student.resultat === "reussi&renvoye" && student.cause
            ? `<p><strong>Cause du renvoi:</strong> ${student.cause}</p>`
            : ""
        }
        <br>
